...
M.form_filemanager.init = function(Y, options) {
    ...
    Y.extend(FileManagerHelper, Y.Base, {
            ...
            if (this.options.subdirs) {
                button_create.on('click',function(e) {
                    ...
                    if (!this.mkdir_dialog) {
                        ...
                        var header =  M.str.moodle.makeafolder;
                        var labelid = 'fm-mkdir_dialog-header';
                        this.mkdir_dialog = new M.core.dialogue({
                            headerContent: '<span id=' + labelid + '>' + header + '</span>',
                            draggable    : true,
                            bodyContent  : node,
                            centered     : true,
                            modal        : true,
        ...
        show_confirm_dialog: function(dialog_options) {
            ...
            var header = (!dialog_options.headerContent ? '' : dialog_options.headerContent);
            this.confirm_dlg.set('headerContent', header);
        ...
        setup_select_file: function() {
            ...
            selectnode.one('.fp-file-delete').on('click', function(e) {
                ...
                dialog_options.headerContent = '<span id="fm-delete-label">' + M.str.moodle.delete + ' ' + fileinfo.fullname + '</span>';
                if (fileinfo.type == 'folder') {
                    params.filename = '.';
